...
· Mostra les característiques principals de l'esquema creat (Subir captura pantalla al repositorio) 
· Explica les diferents taules i variables que hi ha (en el readme que se crea al crear el repositorio)
...
-- La imprimo para verificar que está creada 
SHOW DATABASES;


-- Creo el esquema relacional 
...
-- Formato fecha (da error al ejecutar)
SET TRANSACTION READ WRITE;  
BEGIN WORK;
SET datestyle = YMD;
...
-- Visualizo todas las tuplas de la tabla
select *
...
-- movies.tb_role
-- movies.tb_person
-- movies.tb_movie_person

-- Ejercicio 2
-- Nom, país i data de naixement d'aquelles persones per les quals no consti una data de mort 
...
-- Ejercicio 3
-- Nombre genero y nº total de películas de cada género 
-- Ordenarlo por el orden descendente de nº total de películas  
select movies.tb_genre.genre_name, count(movies.tb_movie.movie_genre_id) as num_peliculas
...
-- Ejercicio 4
-- Nom de cada persona i nº màxim de rols diferents que ha tingut en una mateixa pel·lícula
select tp.person_name, count(tmpr.role_id) as NMaxRoles
from ((movies.tb_movie_person as tmpr 		-- Esta tabla es la única con una primary key (3) referenciadas a las tablas con los datos que necesitamos 
left join movies.tb_role as tr				-- Tabla con roles
on tmpr.role_id = tr.role_id) 			 
left join movies.tb_person as tp			-- Nombre
on tmpr.person_id = tp.person_id)
group by tp.person_name, tmpr.movie_id		-- Agrupa las tuplas/registros/filas con los mismos valores en el atributo count() NMmaxRoles
order by NMaxRoles desc, tp.person_name;	-- Ordenada la tabla de más roles a menos, y dentro de esto, por orden alfabético


-- Posteriorment, mostra únicament aquelles persones que hagin assumit més d'un rol en una mateixa pel·lícula.
select tp.person_name, count(tmpr.role_id) as n_max_rol
from ((movies.tb_movie_person as tmpr 
left join movies.tb_role as tr
on tmpr.role_id = tr.role_id) 
left join movies.tb_person as tp
on tmpr.person_id = tp.person_id)
group by tp.person_name, tmpr.movie_id
having n_max_rol > 1						
order by n_max_rol desc, tp.person_name;


-- Ejercicio 5
-- Crear un nou gènere anomenat "Documental" el qual tingui com a identificador el nombre 69.
...
-- Ejercicio 6
-- Elimina "La Gran Familia Española" (tb_movie_person y tb_movie -> movie_id = 11)
...
-- Compruebo si se ha borrado de tb_movie
select *
from  tb_movie


-- Ejercicio 7
-- Canvia el gènere de la pel·lícula "Ocho apellidos catalanes" perquè consti com a comèdia (no com a romàntica)
